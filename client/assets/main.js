        ...
        <article class="post" data-post-id="1">
            ...
            <div class="post-footer">
                <button class="action-btn like-btn" ${
                  !currentUser
                    ? 'disabled title="Для взаимодействия войдите в систему"'
                    : ""
                }>
...
function loadAuthContent() {
  ...
  let postsHTML = `
        <article class="post" data-post-id="1">
            ...
            <div class="post-footer">
                <button class="action-btn like-btn ${
                  likedPosts.has("1") ? "liked" : ""
                }">
                    <img src="../images/heartt.svg" alt="Лайк" class="icon-heart" />
                    <span class="counter">${
                      likedPosts.has("1") ? "14" : "13"
                    }</span>
                </button>
        ...
        <article class="post" data-post-id="2">
            ...
            <div class="post-footer">
                <button class="action-btn like-btn ${
                  likedPosts.has("2") ? "liked" : ""
                }">
                    <img src="../images/heartt.svg" alt="Лайк" class="icon-heart" />
                    <span class="counter">${
                      likedPosts.has("2") ? "8" : "7"
                    }</span>
                </button>
  ...
  userPosts.forEach((post) => {
    postsHTML += `
            <article class="post" data-post-id="${post.id}">
                <div class="post-header">
                    <img src="${post.avatar || "../images/Profile.svg"}" alt="${
      post.username
    }" class="avatar" />
                ...
                <div class="post-footer">
                    ...
                    <button class="action-btn comment-btn" onclick="location.href='/pages/comments.html?postId=${
                      post.id
                    }'">
                        <span>Комментарии</span>
                        <span class="counter-comment">${
...
function loadProfileContent() {
  ...
  if (postsContainer) {
    const userPostsHTML = userPosts
      ...
      .map(
        (post) => `
            <article class="post profile-post" data-post-id="${post.id}">
                <div class="post-header">
                    <img src="${post.avatar || "../images/Profile.svg"}" alt="${
          post.username
                ...
                <div class="post-footer">
                    ...
                    <button class="delete-btn" title="Удалить пост">
    ...
    postsContainer.innerHTML =
      userPostsHTML ||
      '<p class="no-posts">Вы еще не опубликовали ни одного поста</p>';
  }
...
function loadCommentsContent() {
  ...
  if (postContainer && post) {
    postContainer.innerHTML = `
            <div class="post-header">
                <img src="${post.avatar || "../images/seneka.svg"}" alt="${
      post.username
    }" class="avatar" />
                <h1 class="post-title">${post.username}</h1>
            </div>
  ...
  if (commentsContainer && post) {
    commentsContainer.innerHTML = post.comments?.length
      ? post.comments
          .map(
            (comment) => `
            <article class="comment">
                <div class="comment-header">
                    <img src="${
                      comment.avatar || "../images/Profile.svg"
                    }" alt="${comment.username}" class="avatar-comment" />
...
function getPostById(postId) {
  if (postId === "1") {
    ...
    return {
      ...
      avatar: "../images/seneka.svg",
      content:
        "Говорят, что Гай Цезарь отличался помимо прочих немалочисленных своих пороков каким-то удивительным сладострастием в оскорблениях; ему непременно нужно было на всякого повесить какой-нибудь обидный ярлык...",
      likes: 7,
      comments: [],
    };
  }
...
function setupEventListeners() {
  ...
  document.addEventListener("click", (e) => {
    if (e.target.closest(".like-btn")) {
      ...
      counter.textContent =
        parseInt(counter.textContent) + (likedPosts.has(postId) ? 1 : -1);
      saveAppState();
    }
  });
      ...
      input.onchange = () => {
        if (input.files && input.files[0]) {
          ...
          reader.onload = (event) => {
            ...
            document
              .querySelectorAll(".avatar, .icon-profile")
              .forEach((img) => {
  ...
  // Поиск
  document
    .querySelectorAll(".search-input:not(#search-input)")
    .forEach((input) => {
